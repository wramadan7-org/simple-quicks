...
import { useMenuActions } from "../../stores/menuStore";
import {
  useChatActions,
  useChatIsEdit,
  useChats,
...
export default function InboxPopup() {
  ...
  const { setInboxSelected, setUpdateInboxById } = useInboxActions();
  const { setCloseMenu, setActiveMenu } = useMenuActions();
  const { setSendChat, setUpdateChatById, setIsEdit, setChatSelected } =
    useChatActions();
  ...
  const handleSelectInbox = (inbox: InboxType | null) => {
    if (!inbox) return;

    setInboxSelected(inbox);
    setUpdateInboxById(inbox.idInbox, { status: "read" });
  };
  ...
  const handleClickShowNewMessage = () => {
    if (!inboxSelected) return;

    chats
      ?.filter((item) => item.idInbox === inboxSelected?.idInbox)
      .forEach((item) => {
        setUpdateChatById(item.idChat, { isRead: true });
      });
  ...
  const handleSubmitChat = useCallback(async () => {
    ...
    try {
      ...
      chats
        ?.filter((item) => item?.idInbox === inboxSelected?.idInbox)
        .forEach((item) => {
          setUpdateChatById(item.idChat, {
            isNew: false,
          });
        });

      setUpdateInboxById(inboxSelected.idInbox, {
        ...inboxSelected,
        name: "You",
        status: "read",
        message: messageState,
  ...
  }, [inboxSelected, scrollChatToBottom, setSendChat]);
  ...
  return (
    <motion.div
      ...
      className="w-full h-full flex flex-col max-w-1/3 max-h-3/4 bg-white text-black fixed right-8 bottom-26 z-50 rounded-md py-6"
    >
      {/* Header */}
      {!inboxSelected && (
      ...
      {!isLoadingInboxState && inboxSelected && (
        <div className="flex flex-col w-full h-full text-[#4F4F4F]">
          <div className="px-8 pb-6 flex flex-row flex-nowrap items-center justify-between border-b border-[#BDBDBD] relative">
          ...
          <div
            ref={scrollRef}
            className="flex flex-col h-full overflow-y-scroll px-8 gap-[22px] py-[18px] relative"
          >
            {chats
              ?.filter(
                (item) =>
              ...
              ?.map((item, index, arr) => {
                ...
                return (
                  <Fragment key={item.idChat}>
                    {showLabel && (
                      <div className="flex flex-row items-center justify-center flex-nowrap gap-[22px]">
                        <div
          ...
          <div className="flex flex-row flex-nowrap items-end gap-[13px] px-8">
            <textarea
              ref={inputRef}
